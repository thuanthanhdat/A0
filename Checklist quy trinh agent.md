...
1. **Giai đoạn 1 chưa có artifact thật, tuyệt đối KHÔNG dùng TRANG_THAI.**
2. Giai đoạn 1 chỉ tạo Proposal, phản biện, tổng hợp, đồng thuận, A0 Approval và Execution Specification.
3. TRANG_THAI chỉ bắt đầu dùng từ khi Codex tạo hoặc sửa file thật trong repository ở Giai đoạn 2.
4. **Phản biện song song (Giai đoạn 1):** A Review và TL Review cùng phản biện một Proposal trong cùng một lượt trên không gian hội thoại ngữ nghĩa.
...
6. **Chỉ định đường dẫn tuyệt đối:** Author bắt buộc phải khai báo chính xác `target_path` trong Execution Spec. Codex cấm tự ý tìm kiếm tự do trên repository để tránh lãng phí token.
7. **Báo cáo đường dẫn bắt buộc:** Khi Codex xuất báo cáo (Execution Report), bắt buộc phải liệt kê đầy đủ danh sách đường dẫn file tạo mới (`FILES_CREATED`), sửa đổi (`FILES_MODIFIED`) và file liên quan (`RELATED_FILES`).
8. **Mô tả kết quả rõ ràng:** Không ghi "PASS" chung chung ở phần lý do, phải ghi rõ kết luận chuyên môn chi tiết.
9. **Bàn giao chủ động & Mắt xích quay về chuẩn:** Sau mỗi bước có chuyển giao trách nhiệm, tác nhân đang xử lý phải chủ động phát tin nhắn bàn giao. Mọi báo cáo kết quả thực thi từ Codex BẮT BUỘC trả về `A Review` (`NEXT=Gửi A Review`) để A Review chuyển tiếp định tuyến, tuyệt đối không nhảy cóc.
10. **Ủy quyền Merge rõ ràng:** Codex chỉ tiến hành Merge PR khi thông điệp bàn giao tại bước Pre-merge xác nhận rõ ràng thẩm quyền merge và điều kiện merge đã đạt.
...
Chủ sở hữu bước tiếp theo: A Review và TL Review (Phản biện song song).
...
OWNER=Codex
ACTION=Kiểm tra repository tại target_path -> Tạo/Sửa file -> Tự kiểm tra (Self-validation) -> Tạo branch -> Commit -> Push -> Mở PR -> Lập Execution Report (đính kèm đầy đủ danh sách đường dẫn FILES_CREATED, FILES_MODIFIED, RELATED_FILES) -> Báo cáo hoàn thành tới A Review.
NEXT=Gửi A Review
2.2. Codex thực thi chuỗi Git
Bắt đầu dùng TRANG_THAI từ bước này vì file thật đã xuất hiện trong repository.

TRẠNG_THÁI: TAI_LIEU_DA_HINH_THANH_TREN_BRANCH_HOAC_PR
...
Chủ sở hữu bước tiếp theo: A Review (Mắt xích bắt buộc: Codex báo cáo về A Review để A Review kiểm tra routing sang Author cho Pre-merge verification).
...
OWNER=Codex
ACTION=Codex báo cáo hoàn thành, cung cấp PR link, commit SHA, PR diff và danh sách đầy đủ đường dẫn file để A Review xác nhận và định tuyến sang Author tiến hành Pre-merge verification.
NEXT=Gửi A Review
...
OWNER=Author
ACTION=Author kiểm tra lại lý do lỗi từ báo cáo; tiến hành sửa Execution Specification hoặc quay lại A0 nếu cần thay đổi ngữ nghĩa.
NEXT=Gửi A Review
3. NGHIỆM THU, BẰNG CHỨNG VÀ CHUYỂN BƯỚC (GIAI ĐOẠN 3)
3.1. Author kiểm trước merge (Pre-merge Verification)
TRẠNG_THÁI: DANG_KIEM_TRA_PR_TRUOC_MERGE

Ai làm: Author (sau khi nhận định tuyến từ A Review).
...
Chủ sở hữu bước tiếp theo: Codex (Nếu PR Đạt - Cấp ủy quyền Merge) HOẶC Codex (Nếu PR Chưa đạt - Yêu cầu sửa).
...
OWNER=Codex
ACTION=Xác nhận ủy quyền merge: Codex tiến hành Merge PR vào main; sau đó thực hiện verify main SHA, target file status và báo cáo kết quả về A Review.
NEXT=Gửi A Review
...
OWNER=Codex
ACTION=Codex sửa lại file trên branch theo đúng Correction Specification do Author cấp; cấm sửa ngoài phạm vi; báo cáo lại kết quả về A Review kèm danh sách đường dẫn file cập nhật.
NEXT=Gửi A Review
3.2. Codex merge và verify main
TRẠNG_THÁI: DA_MERGE_VA_VERIFY_MAIN HOẶC MERGE_HOAC_VERIFY_BI_CHAN

Ai làm: Codex (khi đã nhận ủy quyền merge rõ ràng tại bước 3.1).
...
Chủ sở hữu bước tiếp theo: A Review (Mắt xích bắt buộc: Báo cáo kết quả post-merge về A Review để A Review định tuyến sang Author nghiệm thu cuối).
...
OWNER=Codex
ACTION=Báo cáo đầy đủ bằng chứng post-merge (Main SHA, verified paths) để A Review xác nhận và định tuyến cho Author tiến hành nghiệm thu kỹ thuật lần cuối.
NEXT=Gửi A Review
...
TRẠNG_THÁI: DANG_NGHIEM_THU_CUOI

Ai làm: Author (sau khi A Review chuyển tiếp kết quả post-merge từ Codex).
...
OWNER=Codex
ACTION=Codex tiến hành sửa đổi theo đúng Correction Specification do Author phát hành; cấm mở thêm semantic mới; báo cáo lại về A Review.
NEXT=Gửi A Review
