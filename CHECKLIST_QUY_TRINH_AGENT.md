...
Giai đoạn 1 chưa có artifact thật, tuyệt đối KHÔNG dùng TRANG\_THAI.

Giai đoạn 1 chỉ tạo Proposal, phản biện, tổng hợp, đồng thuận, A0 Approval và Execution Specification.

TRANG\_THAI chỉ bắt đầu dùng từ khi Codex tạo hoặc sửa file thật trong repository ở Giai đoạn 2.

Phản biện song song: A Review và TL Review cùng phản biện một Proposal trong cùng một lượt.
...
Chỉ định đường dẫn tuyệt đối: Author bắt buộc phải khai báo chính xác target\_path trong Execution Spec. Codex cấm tự ý tìm kiếm tự do trên repository để tránh lãng phí token.
...
Định danh minh bạch (SENDER): Mọi khối tin nhắn bàn giao bắt buộc phải khai báo trường SENDER ở đầu khung để xác định chính xác tác nhân phát tin nhắn.

QUY CÁCH KHỐI TIN NHẮN BÀN GIAO CHUẨN (ROLE\_ROUTING\_AND\_HANDOFF\_RULE)

Mọi khối tin nhắn bàn giao bắt buộc gồm đúng 6 dòng và bắt đầu bằng nhãn định hướng chỉ định:

Plaintext

Gửi <Tên\_Tác\_Nhận\_Kế\_Tiếp>:

SENDER=<Tác\_nhân\_phát\_tin\_nhắn>

RESULT=<Enum\_Chuẩn\_Duy\_Nhất>

REASON=<Mô\_tả\_chi\_tiết\_lý\_do\_bằng\_tiếng\_Việt>

OWNER=<Role\_Duy\_Nhất\_Nắm\_Bóng>

ACTION=<Mô\_tả\_chi\_tiết\_hành\_động\_cần\_làm\_bằng\_tiếng\_Việt>

NEXT=<Hướng\_Đi\_Chuẩn\_Duy\_Nhất>
...
RESULT=PASS\_BEFORE\_A0\_APPROVAL

REASON=Proposal đã soạn xong đầy đủ mục tiêu, phạm vi, dependency và rủi ro; sẵn sàng để phản biện song song.

OWNER=A Review

ACTION=A Review chủ trì lượt phản biện (phối hợp cùng TL Review) để đánh giá Proposal: A Review soi governance, scope, authority, risk; TL Review soi kiến trúc, kỹ thuật, vận hành; nêu rõ nội dung đồng thuận, chưa đồng thuận và blocker.
...
ACTION=A Review chủ trì (cùng TL Review) tiếp tục phản biện tập trung CHỈ trên danh sách các issue còn mở; đề xuất cách sửa để đạt hội tụ; không mở lại các phần đã đồng thuận.
...
ACTION=A0 xem xét bản Final Synthesis và ra quyết định Phê duyệt (PASS) hoặc Không phê duyệt (NOT\_PASS) về mặt ngữ nghĩa.
...
REASON=Author đã cập nhật danh sách các issue chưa đồng thuận cho vòng lặp tiếp theo; đã loại bỏ các nội dung vừa đạt đồng thuận.

OWNER=A Review

ACTION=A Review chủ trì (cùng TL Review) tiếp tục phản biện tập trung vào các issue còn tồn đọng cho đến khi không còn blocker.
...
RESULT=PASS\_AFTER\_A0\_APPROVAL

REASON=A0 đã phê duyệt chính thức quyết định ngữ nghĩa. Toàn bộ nội dung đã được khóa chặt, cấm thêm semantic mới.

OWNER=Author

ACTION=Author lập Execution Specification chi tiết (chỉ thị kỹ thuật không mập mờ, ghi rõ target\_path cụ thể) để giao Codex khởi tạo/sửa tài liệu thật trên Git.
...
REASON=Execution Specification đã lập xong với đầy đủ target\_path chuẩn xác; đủ điều kiện để Codex thao tác trên Repository mà không cần duyệt tìm kiếm tự do.

OWNER=Codex

ACTION=Kiểm tra repository tại target\_path -> Tạo/Sửa file -> Tự kiểm tra (Self-validation) -> Tạo branch -> Commit -> Push -> Mở PR -> Lập Execution Report (đính kèm đầy đủ danh sách đường dẫn FILES\_CREATED, FILES\_MODIFIED, RELATED\_FILES) -> Dừng và báo cáo A Review.
...
FILES\_CREATED: Danh sách đường dẫn file tạo mới.

FILES\_MODIFIED: Danh sách đường dẫn file chỉnh sửa.

RELATED\_FILES: Danh sách đường dẫn file liên quan.

Bằng chứng Commit SHA, Blob SHA, PR link.

Chủ sở hữu bước tiếp theo: A Review (để định tuyến sang Author).
...
RESULT=CODEX\_COMPLETED

REASON=Codex đã hoàn tất thao tác trên Git; Execution Report đã đính kèm đầy đủ danh sách đường dẫn FILES\_CREATED, FILES\_MODIFIED, RELATED\_FILES và bằng chứng commit/PR.

OWNER=Codex

ACTION=Codex báo cáo hoàn thành, cung cấp PR link, commit SHA, PR diff và danh sách đầy đủ đường dẫn file về cho A Review để A Review định tuyến bàn giao bóng sang cho Author tiến hành Pre-merge verification.
...
RESULT=TECHNICAL\_FAIL

REASON=Codex phải dừng thực thi do Spec mập mờ, sai target\_path, thiếu authority, vượt scope hoặc mâu thuẫn với artifact hiện hữu trên main.
...
REASON=PR diff và danh sách đường dẫn file báo cáo hoàn toàn khớp với target\_path trong Execution Spec, đúng scope và không có out-of-scope change; đủ điều kiện merge.

OWNER=Codex

ACTION=Ủy quyền chính thức: Codex tiến hành Merge PR #<PR\_NUMBER> vào main; sau đó thực hiện verify main SHA, target file status và báo cáo kết quả.
...
REASON=PR chưa đạt do sửa sai target\_path, phát sinh file thay đổi ngoài phạm vi (out-of-scope change), sai nội dung hoặc thiếu bằng chứng evidence.
...
Làm như thế nào: Merge PR theo ủy quyền. Sau khi merge, kiểm tra main commit SHA, target file status, blob SHA, target path và nội dung file thực tế trên main.

Chủ sở hữu bước tiếp theo: A Review (sau đó định tuyến sang Author nghiệm thu).
...
REASON=PR đã được merge thành công vào main; đã xác minh main commit SHA, blob SHA và file status tại target\_path hoàn toàn hợp lệ.

OWNER=Codex

ACTION=Báo cáo bằng chứng post-merge lên hệ thống về cho A Review để A Review định tuyến bàn giao bóng sang cho Author tiến hành nghiệm thu kỹ thuật lần cuối.
...
REASON=Nghiệm thu cuối thất bại do Codex thực hiện sai/thiếu spec hoặc nội dung trên main tại target\_path không khớp bằng chứng evidence.
...
REASON=Execution đã hoàn tất xuất sắc, đúng target\_path và được verify trên main; bước tiếp theo yêu cầu chỉ đạo chiến lược hoặc A0 Gate.
...
Chủ sở hữu bước tiếp theo: A0 (nếu cần quyết định mới) HOẶC A Review (nếu successor đã rõ để phản biện bước tiếp).
...
REASON=Work Item đã đóng Checkpoint bằng đầy đủ bằng chứng Git và danh sách target\_path đã verify; Work Item tiếp theo cần A0 duyệt Gate hoặc hướng đi mới.
